...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct  3 18:23:24 2025
...
class Plasma(element.Element):
    ...
    def initialize_plasma(self):
        """ Save plasma and number densities, passed as an array, to file. """
        self.n= self.n0*np.ones((self.Nx, self.Ny, self.Nz), dtype='double')
        self.ne= np.zeros((self.Nx, self.Ny, self.Nz), dtype='double')
        for i in range(self.Nz):
            self.save_num_density(self.n[:, :, i], i)
            self.save_plasma_density(self.ne[:, :, i], i)
    ...
    def get_ionization_energy(self):
        ...
        return self.get_plasma_number()*self.gas.EI*1.602e-19
    ...
    def plot_propagation_array(self, propagation_result, axis= 'x', xylim= None):
        ...
        plt.xlabel('z (m)')
        plt.ylabel(axis+ ' (um)')
        plt.colorbar(label= 'Plasma Density ($10^{16} cm^{-3}$)')
        plt.twinx()
        plt.plot(z_grid, propagation_result[int(propagation_result.shape[0]//2), :]*10, \
                'w--')
