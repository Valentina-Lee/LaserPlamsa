...
class propagation_GPU():
    ...
    def nonlinear_propagate(pulse, material, thickness, z_steps,
                                   GVD=True, Diffraction=True, SPM_Kerr=True, save= False, noteName='', absStart= 0):
        """
        ...
        gvd_phase = cp.exp(-0.5j * material.beta2 * omega**2 * dz)
        ...
        for s in range(z_steps):
            ...
            if SPM_Kerr:
                intensity = 0.5 * constants.c * constants.epsilon_0 * cp.abs(E_field)**2
